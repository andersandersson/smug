...
LinkedList* BinarySearchTree_removeIf(BinarySearchTree* tree, BOOL (*predicate)(void*, void*), void* param)
{
    ...
    result = BinarySearchTree_findIf(tree, predicate, param);

    for(node = result->first; node != NULL; node = node->next)
    {
        BinarySearchTree_remove(tree, node->item);
    }
...
LinkedList* BinarySearchTree_findIf(BinarySearchTree* tree, BOOL (*predicate)(void*, void*), void* param)
{
    ...
    // Iterate over the tree and filter out elements matching the predicate
    for(BinarySearchTreeIterator_reset(tree, &iter);
        BinarySearchTreeIterator_valid(&iter);
        BinarySearchTreeIterator_step(&iter))
    {
        void* item = BinarySearchTreeIterator_get(&iter);

        if(TRUE == predicate(param, item))
        {
            LinkedList_addLast(result, item);
        }
    }
